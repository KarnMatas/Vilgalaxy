...
def edgedetect(frame):
    ...
    edge = cv2.Canny(average, 50, 80)  # 10 71c  # ส่องไฟ 6 98     50  80
    return edge
...
def convert_coordinate(lstpixel):
    yworld = (lstpixel[1] / 540) * 390
    xworld = (lstpixel[0] / 540) * 390
    # yworld = (yworld - 370) * (-1)
    # xworld = (xworld - 370) * (-1)
    return [xworld,yworld]
def PathFinder(frame,tune1,tune2):
    global cornerpoints_skel
    clone = frame.copy()[30:-30, 30:-30]
    imgGrey = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    newimg = imgGrey[30:-30, 30:-30]  #40:-86, 40:-55
    newgrey = newimg.copy()
